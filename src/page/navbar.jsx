...
export default function Navbar() {
  ...
  const handleLogout = () => {
    // Xóa dữ liệu xác thực từ localStorage
    localStorage.removeItem('authData');

    // Hiển thị thông báo
    toast.info('Bạn đã đăng xuất. Vui lòng đăng nhập lại.');
    ...
    <ul className="flex flex-col mt-4 font-medium md:flex-row md:mt-0 md:space-x-8 rtl:space-x-reverse">
      ...
      <li>
        ...
        <div
          ...
          className={`absolute z-10 grid ${isDropdownOpen ? '' : 'hidden'} grid-cols-2 text-sm bg-white border border-gray-100 rounded-lg shadow-md dark:border-gray-700 md:grid-cols-3 dark:bg-gray-700`}
        >
            ...
            <ul className="space-y-4">
              <li>
                <Link
                  ...
                  className="flex items-center text-gray-500 dark:text-gray-400 hover:text-blue-600 dark:hover:text-blue-500 group"
                >
                  <span className="sr-only">Google Cache</span>
                  <svg
                    className="w-3 h-3 me-2 text-gray-400 dark:text-gray-500 group-hover:text-blue-600 dark:group-hover:text-blue-500"
                    aria-hidden="true"
                    xmlns="http://www.w3.org/2000/svg"
